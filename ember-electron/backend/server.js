...
class Server {
    constructor(nextlinuxPath, port, hostname) {
        this.port = port || 3000;
        this.hostname = hostname ||  'localhost';
        this.nextlinuxController = controller(nextlinuxPath);
    }
    ...
    _listViews(request, response) {
        ...
        return this.nextlinuxController.runCnextlinux(args, response);
    }

    _loadViews() {
        let args = ['--list-views', '-j'];

        return this.nextlinuxController.runCnextlinux(args);
    }

    _getView(request, response) {
        let fileName = untildify(request.params.fileName);
        let viewInfo = JSON.parse(request.params.view);
        let args = ['-r', path.resolve(this.nextlinuxController.nextlinuxPath, fileName), '-v', viewInfo.id, '-j', '-pc'];
        ...
        response.setHeader('Content-Type', 'application/json');
        this.nextlinuxController.runCnextlinux(args, response)
            .catch((error) => {
                console.error('cnextlinux execution failed', error);
            })
        ;
}
    ...
    _getSummary(request, response) {
        ...
        const args = ['-r', path.resolve(this.nextlinuxController.nextlinuxPath, fileName), '-c', 'wnextlinux_summary'];
        ...
        this.nextlinuxController.runNextlinux(args, response)
            .catch((error) => {
                console.error('nextlinux execution failed', error);
            })
        ;
    }
}

function createServer(nextlinuxPath, port, hostname) {
    return new Server(nextlinuxPath, port, hostname);
}
